...
for _,path in ipairs(files) do
  if fs.exists(path) and (path:find("config.lua") or path=="startup.lua") then
    print("Keeping existing "..path)
  else
    local dir=fs.getDir(path)
    if dir~="" and not fs.exists(dir) then fs.makeDir(dir) end
    local tmp=path..".installing"
    if fs.exists(tmp) then fs.delete(tmp) end
    print("Downloading "..path)
    local called,result=pcall(shell.run,"wget",BASE_URL..path,tmp)
    if not called or result==false or not fs.exists(tmp) then
      if fs.exists(tmp) then fs.delete(tmp) end
      error("download failed for "..path..": "..tostring(result))
    end
